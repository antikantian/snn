...
import snn.matrix.Constants._

/**
trait Matrix[A] { self =>
  def mulmatrix(xa: Matrix[A]): Matrix[A]
  def eltwiseProduct(xa: Matrix[A]): Matrix[A]
  def mapMatrix(f: A => A): Matrix[A]
  def transpose: Matrix[A]
  def sub(xa: Matrix[A]): Matrix[A]
  def scale(a: A): Matrix[A]

  /** Matrix multiplication */
  def *(xa: Matrix[A]): Matrix[A] =
    mulmatrix(xa)

  /** Element-wise multiplication, a.k.a. Schur product or Hadamard product */
  def :*(xa: Matrix[A]): Matrix[A] =
    eltwiseProduct(xa)
